package club.zhisimina.templateapp.customerview.beforehandappoint;
...
import club.zhisimina.templateapp.adapter.basicsset.ItemDeleteRCVAdapter;
import club.zhisimina.templateapp.util.LogUtils;


/**
 * @description  列表左滑显示删除按钮
 * @author  fupengpeng
 * @date  2018/2/28 0028 15:03
 */

public class ItemDeleteRCV extends RecyclerView {
    ...
    //构造函数
    public ItemDeleteRCV(Context context) {
        this(context, null);
    }

    public ItemDeleteRCV(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ItemDeleteRCV(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mMenuState == MENU_CLOSED) {  //关闭状态
                    ...
                    //获得这个view的ViewHolder
                    ItemDeleteRCVAdapter.Holder holder = (ItemDeleteRCVAdapter.Holder) getChildViewHolder(view);
                    //获得这个view的position
                    mPosition = holder.getAdapterPosition();
                    //获得这个view的整个布局
                    mItemLayout = holder.llAtvtBasetTimePrice;
                    Log.e("-----sssssssss", "" + mItemLayout);
                    //获得这个view的加入购物车按钮
                    tvDelete = holder.tvAtvtBasetTimePriceItemDelete;
